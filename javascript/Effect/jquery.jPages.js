	...
	(function ($, window, document, undefined) {
		...
		Plugin.prototype = {
			...
			writeNav : function () {
				...
				for (; i <= this._numPages; i++) {
					if (i === 1 && this.options.startRange === 0)
						navhtml += (this.options.navLi) ? "<li'><span>...</span></li>" : "<span>...</span>";
					if (i > this.options.startRange && i <= this._numPages - this.options.endRange)
						navhtml += (this.options.navLi) ? "<li class='jp-hidden itemLi'><a href='#'>" : "<a href='#' class='jp-hidden itemLi'>";
					else
						navhtml += (this.options.navLi) ? "<li class='itemLi'><a href='#'>" : "<a class='itemLi'>";
					...
					navhtml += (this.options.navLi) ? "</a></li>" : "</a>";
					if (i === this.options.startRange || i === this._numPages - this.options.endRange)
						navhtml += (this.options.navLi) ? "<li><span>...</span></li>" : "<span>...</span>";
				}
			...
			writeBtn : function (which) {
				var whichButton = (this.options.navLi) ? "<li><a class='jp-" + which + "' href='#'>" + this.options[which] + "</a></li>" : "<a class='jp-" + which + "'>" + this.options[which] + "</a>";
				return this.options[which] !== false && !$(this["_" + which]).length ? whichButton : "";
			},
			...
			updateBtns : function (nav, page) {
				if (page === 1) {
					if (this.options.navLi) {
						nav.first.closest('li').addClass("jp-hidden");
						nav.previous.closest('li').addClass("jp-hidden");
					}else{
						nav.first.closest('li').addClass("jp-hidden");
				...
				if (page === this._numPages) {
					if (this.options.navLi) {
						nav.next.closest('li').addClass("jp-hidden");
						nav.last.closest('li').addClass("jp-hidden");
					}else{
				...
				if (this._currentPageNum === 1 && page > 1) {
					if (this.options.navLi) {
						nav.first.closest('li').removeClass("jp-hidden");
						nav.previous.closest('li').removeClass("jp-hidden");
					}else{
				...
				if (this._currentPageNum === this._numPages && page < this._numPages) {
					if (this.options.navLi) {
						nav.next.closest('li').removeClass("jp-hidden");
						nav.last.closest('li').removeClass("jp-hidden");
					}else{
			...
			updateCurrentPage : function (nav, page) {
				...
				if (this.options.navLi)
					if (this.options.navLi)
						nav.currentPage.closest('li').removeClass("active");
				nav.currentPage = nav.pages.eq(page - 1).closest('li').addClass("active");
			...
			updateBreaks : function (nav, interval) {
				if (interval.start > this.options.startRange || (this.options.startRange === 0 && interval.start > 0))
					if (this.options.navLi)
						nav.fstBreak.closest('li').removeClass("jp-hidden");
					else
						nav.fstBreak.removeClass("jp-hidden");
				else if (this.options.navLi)
					nav.fstBreak.closest('li').addClass("jp-hidden");
				else
					nav.fstBreak.addClass("jp-hidden");
				if (interval.end < this._numPages - this.options.endRange)
					if (this.options.navLi)
						nav.lstBreak.closest('li').removeClass("jp-hidden");
					else
						nav.lstBreak.removeClass("jp-hidden");
				else if (this.options.navLi)
					nav.lstBreak.closest('li').addClass("jp-hidden");
				else
	...
	(function ($) {
		$.extend($.fn, {
			jPage : function (options) {
				var dom = this,
				config = {
					pageJump : 'select[name=pageJump]',
					legend : '#pageInfo',
					navLi : true,
					css : 'sea.css',
					callback : function (pages, items) {
						$(config.pageJump).empty();
						for (var i = 1; i <= pages.count; i++) {
							if (i == pages.current) {
								$('<option value="' + i + '" selected>第' + i + '页</option>').appendTo(config.pageJump);
							} else {
								$('<option value="' + i + '">第' + i + '页</option>').appendTo(config.pageJump);
							}
						}
						var html = '当前第' + pages.current + '页，页面记录：第' + items.range.start + '至第';
						html += items.range.end + '条，总记录：' + items.count + '条，总共' + pages.count + '页';
						$(config.legend).html(html);
					}
				};
				$.seaBase.run(config, options, function (config) {
					$(dom).jPages(config);
					if (config.pageJump != undefined) {
						$(config.pageJump).change(function () {
							$(config.pagination).jPages(parseInt($(this).val()));
						});
					}
				});
			}
