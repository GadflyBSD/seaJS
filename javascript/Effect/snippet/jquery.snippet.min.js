		...
		$.extend($.fn, {
			seaSnippet: function(options){
				...
				$.seaBase.run(config, options, function(config){
					if(config.target){
						var code = $('<'+config.target+' />').text($(config.target).html()).html();
						$(dom).append(code);
					}
					$(dom).snippet(config.language, config);
				});
			}
		});
	}(jQuery));
