...
(function($) {
    ...
    var layoutManager = {
        ...
        defaultConfig: {
            animate: false,
            cellW: 160, // function(container) {return 100;}
            cellH: 160, // function(container) {return 100;}
            delay: 30, // slowdown active block;
    ...
    */
	...
	$.fn.Freewall = function(options){
		var config = {},dom = this,
			methods = ($(this).data('methods'))?$(this).data('methods'):'fitZone';
		$.seaBase.run(config, options, function(config){
			var wall = new freewall(dom);
			wall.reset(config);
			switch (methods){
				case 'fitImage':
				...
				case 'fitZone':
				default:
					wall.fitZone($(dom).width() , $(dom).height());
					break;
